...
const connectDB = async () => {
  try {
    const conn = await mongoose.connect(
      "mongodb+srv://<username>:<password>@cluster0.lgkokgf.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
      {
...
app.post("/admin/login", async (req, res) => {
  ...
  await adminModel.findOne({ adminId: adminId }).then((user) => {
    console.log(user);
    if (user) {
